        ...
        function draw2D() {
            ...
            segments.forEach(seg => {
                ...
                // Draw Cabinets
                const segCabs = cabinets.filter(c => c.segment === seg.id && c.kind !== 'EMPTY_BASE' && c.kind !== 'EMPTY_WALL');
                const legH_mm = 100;
                segCabs.forEach(cab => {
        ...
        document.addEventListener('mousemove', e => {
            if (draggedApplId) {
                ...
                const others = appliances.filter(a => {
                    if (a.segment !== appl.segment || a.id === appl.id || a.id === draggedPairedApplId) return false;
                ...
                for (let o of others) {
                    ...
                    if (Math.abs(newStart - oEnd) < SNAP_DIST) {
                        finalStart = oEnd;
                    }
        ...
        function init3D() {
            ...
            function create3DCabinet(cab) {
                ...
                if (cab.kind === 'EMPTY_BASE' || cab.kind === 'EMPTY_WALL') {
                    return group;
                }
